    ...
    return (
        <header className="sticky top-0 z-40 border-b border-border bg-surface/70 backdrop-blur-xl">
            <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
                ...
                {/* Desktop */}
                <div className="hidden items-center gap-4 md:flex hover:cursor-pointer">
                    {isAuthenticated ? (
                        <UserProfile />
                    ) : (
                        <>
                            ...
                            <Link to="/signup">
                                <Button variant="dark" size="sm" className="hover:cursor-pointer">
                                    Get tickets
                                </Button>
                            </Link>
                        </>
            ...
            <div
                className={`overflow-hidden border-t border-border transition-all duration-300 ease-in-out md:hidden ${
                    open ? "max-h-64 border-t opacity-100" : "max-h-0 border-t-0 opacity-0"
                }`}
            >
